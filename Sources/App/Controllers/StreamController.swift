...
struct StreamController: RouteCollection {
    func boot(routes: RoutesBuilder) throws {
        let stream = routes.grouped("stream")
        stream.get(use: index)
        stream.on(.POST, body: .stream, use: upload)
        stream.group(":fileID") { builder in
            builder.get(use: getOne)
            builder.get("download", use: downloadOne)
        }
